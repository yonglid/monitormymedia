...
class Home extends React.Component {
 ...
 async componentDidMount() {
  ...
  //  );
   this.apiClient.getStarred().then((data) =>
     this.setState({...this.state, starred: data})
   );
 }
  ...
  return articles.map((article) => {
    return (
      <Grid item xs={12} md={3} key={article.id}>
        <GoogleRssArticle onStarred={this.onStarred} isStarred={this.isStarred(article)} article={article} />
      </Grid>
    );
  })
}
